...
@section('content')
    <div class="content2">
        ...
        <div class="button-container">
            <a href="{{ url('/procedimiento') }}" class="button-izquierda arrow-left"><i
                    class="fa-solid fa-circle-arrow-left"></i> Regresar</a>
            @if (auth()->user()->hasRole(['superAdmin', 'administrador', 'tecnico']))
                <a href="{{ route('createTipoP') }}" class="button-derecha"><i class="fas fa-file"></i> Nuevo Tipo de
                    Procedimiento</a>
            @endif
        </div>
                                ...
                                @if (auth()->user()->hasRole(['superAdmin', 'administador']))
                                    <td>
                                        @if (auth()->user()->hasRole(['superAdmin', 'administrador']))
                                            <a class="edit-button"
                                                href="{{ route('editTipoP', ['id' => $tipoProcedimientos->idTipoProcedimiento]) }}"
                                        ...
                                        @if (auth()->user()->hasRole(['superAdmin']))
                                            <button type="button" class="delete-button" title="Eliminar"
                                                ...
                                                <i data-id="{{ $tipoProcedimientos->idTipoProcedimiento }}"
                                                    data-name="{{ $tipoProcedimientos->tipo }}" class="fas fa-trash-alt">
                                                </i>
                                            </button>
                                        @endif
                                    </td>
